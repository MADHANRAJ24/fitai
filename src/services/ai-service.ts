...
export class AIService {
    private static readonly API_KEY = process.env.GROK_API_KEY || process.env.NEXT_PUBLIC_GROK_API_KEY || ""
    ...
    static async generateResponse(message: string, persona: Persona = 'supportive'): Promise<string> {
        if (!this.API_KEY || this.API_KEY.includes("YOUR_")) {
            console.warn("Missing Grok API Key")
        ...
        try {
            ...
            return data.choices?.[0]?.message?.content || "No response computed."
